...
ct.inputs.addAction('Jump', [{"code":"keyboard.KeyW"},{"code":"keyboard.ArrowUp"},{"code":"keyboard.Space"}]);
...
ct.rooms.templates['Level_01'] = {
    ...
    if (!ct.room.paused) {
        ...
        this.unpauseHint = ct.templates.copy('PauseHint', this.player.x, this.player.y);
        this.player.speed = 0;
        ...
        this.player.speed = this.player.constSpeed
...
ct.sound.stop('lvl1')
ct.sound.spawn('lvl1', {volume:0.4})
    },
    extends: {}
}
ct.rooms.templates['Level_02'] = {
    ...
    height: 720,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":64,"y":512,"exts":{},"template":"Rocks_Platform"},{"x":64,"y":256,"exts":{},"template":"Katya"},{"x":128,"y":512,"exts":{},"template":"Spikes"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[{"depth":-10,"tiles":[],"extends":{}}]'),
    backgroundColor: '#000000',
...
for (var i = 0; i < Math.abs(this.vspd); i++) {
    ...
    onCreate: function () {
        ct.room.player = this;
this.constSpeed = 4 * ct.delta; // Максимальная скорость движения по горизонтали
...
ct.templates.templates["Water_Top"] = {
    depth: 0,
    blendMode: PIXI.BLEND_MODES.NORMAL,
    playAnimationOnStart: false,
    texture: "Water_Top",
...
if (ct.room.nextRoom && ct.room.rings == ct.templates.list['RingWidget'].length) {
    if (ct.place.meet(this, this.x, this.y, 'Katya')) {
        ct.sound.spawn('round');

        if(ct.room.crystals == ct.room.crystalsTotal) {
            ct.plusLvl = true;
        }
...
ct.templates.templates["MerryText"] = {
    ...
    onCreate: function () {
        this.text = new PIXI.Text('Are you merry me?', ct.styles.get('HeartStyle'));
this.text.x = + 52;
this.text.anchor.y = 0.5;
this.text.anchor.x = 1;
